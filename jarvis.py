    ...
    try:
        ...
        if "system level" in mode:
            ...
            if weather_data.json()['cod'] == '404':
                ...
                speak(f"The temperature in {user_input}, {country} is: {temp}ºCelsius")

        elif "lame joke" in mode:
            url = 'https://official-joke-api.appspot.com/random_joke'

            # Send a GET request to fetch the joke
            ...
            if response.status_code == 200:
                ...
                speak(f"Here's a joke for you:\n{joke['setup']}")
                print(f"{joke['punchline']}")
                speak(f"{joke['punchline']}")
                speak("HAHAHAHA")
                speak("If I made u fell bad sorry")
                speak("STUF  u deserve this joke")
            else:
